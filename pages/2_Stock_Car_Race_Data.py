...
if etapa_escolhida != "Select a round...":
    ...
    if corrida_label != "Select a race...":
        ...
        if option == "Chart":
            ...
            # --- Last Lap Diff Graph ---
            with tabs[0]:
                graf7 = px.line(sessao_filtrado, x="Lap", y= "Last Lap Diff", color="Driver", title='Last Lap Difference')
                st.plotly_chart(graf7)
        
            # --- Fast Lap Diff Graph ---
            with tabs [1]:
                graf8 = px.line(sessao_filtrado, x="Lap", y= "Fast Lap Diff", color="Driver", title='Fast Lap Delta')
